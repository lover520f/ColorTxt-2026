...
| `legadoAsyncJs.ts` | 自动 `await java.ajax` / `java.post` / HTTP 形态 `java.get`；将含 `await` 的函数提升为 `async`，并对 jsLib 中的异步函数在规则脚本里注入 `await`；括号匹配跳过注释/正则（避免大 jsLib 提升死循环）；Rhino 箭头参数、`return` 补全（不误改 `continue`/`break`；尾部 `(()=>{…})();` IIFE 须 `return (()=>…)()`，勿 `return })();` 触发 ASI） |
