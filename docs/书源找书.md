...
> 亦可：桌面快捷方式、`--find-book` / `--find-book=bookshelf|search|discover` 启动。  
...
两个「更多」：阅读器**顶栏**（书籍信息 / 编辑书源 / 清除缓存）vs **阅读工具栏**（窄屏版式与设置）。勿与主应用「清除缓存」（清 localStorage）混淆。

### 2.2 典型流程

**搜索** — `searchService` 多源并发 → `searchEvent` 流式推送。  
**发现** — `exploreKinds` → `webBook.exploreBook`（`ruleExplore`）。  
**详情 → 目录 → 正文** — `getBookInfo` / `getChapterList` / `getChapterContentWithCache`（读写 `book_cache`）。  
**整书下载** — 先缓存全部非分卷章，再导出 UTF-8 `.txt`（正文非空段首「　　」；标题不加）。  
**书架开读** — `bookshelfOpenReader`：有本地目录缓存可加速；否则拉目录后进阅读器。
...
| `bookSource:download` + `downloadEvent` | 整书下载（先缓存再导出，可取消） |
| `bookSource:exploreKinds/exploreBooks` | 发现 |
| `bookSource:getBookInfo/getChapterList` | 详情与目录 |
| `bookSource:getChapterContent` | 章节正文（命中 book_cache 则离线） |
...
- **编辑**：分标签表单（基本 / 搜索 / 发现 / 详情 / 目录 / 正文…）；阅读器「更多 → 编辑书源」默认打开「正文」。
...
- 点封面/标题可直接开读；行「更多」可进书籍信息、更新、移除等。
- 工具栏：筛选、排序、全部更新、书架管理、更新日志。

### 6.6 书籍信息（详情）

- 封面、简介、目录（正/倒序）、加书架、开始/继续阅读、一键下载。
- 目录项：最后阅读标记、已缓存勾标、下载中旋转刷新。
- 「更多」：刷新、复制链接、变量、编辑书源、**清除缓存**（warning）。

### 6.7 在线阅读器

- 复用主应用阅读能力：字体/行距/主题/语音/全屏/定时滚屏等（找书阅读设置）。
- 侧栏目录同步缓存勾标；阅读命中缓存不写重复联网。
- 顶栏「更多」：书籍信息、编辑书源（定位正文）、清除缓存。

### 6.8 缓存与下载（易混点）

| 概念 | 位置 | 作用 |
|------|------|------|
| **章节缓存目录** | 找书设置 → 下载 → 缓存目录 | `book_cache/`，阅读与整书下载共用 |
...
| 清除缓存 UI | `services/clearBookChapterCache.ts` |
| 找书设置 | `useFindBookSettings.ts`、`FindBookSettings*.vue` |
| 搜索 / 下载 | `searchService.ts`、`downloadService.ts` |
| 章节缓存 | `chapterCache.ts`、`getChapterContentWithCache.ts` |
