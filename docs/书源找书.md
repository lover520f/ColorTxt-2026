...
| `legadoAsyncJs.ts` | 自动 `await java.ajax` / `java.post`；Rhino 箭头参数、`return` 补全（不误改 `continue`/`break`；尾部 `(()=>{…})();` IIFE 须 `return (()=>…)()`，勿 `return })();` 触发 ASI） |
...
- **编辑**：分标签表单（基本 / 搜索 / 发现 / 详情 / 目录 / 正文…）；阅读器顶栏「编辑书源」默认打开「正文」；「更多」含 **登录**（有 `loginUrl` 时）、**搜索**（限定该书源并关闭书籍信息/阅读器等叠层）、清除 Cookie、设置源变量。
...
- 展开书源行：右侧 **搜索**（限定该书源找书）、**更多**（编辑书源 / 置顶到书源列表）；未展开时仅 hover / 聚焦时显示。
