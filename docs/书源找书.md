...
1. **模式**：
   ...
   - 从 URL 回填 `bid` 时：勿覆盖已有 `java.put('bid',…)`；且须剥掉 `bid=bid=hash`
...
| `legadoJsoupShim.ts` | `org.jsoup.Jsoup.parse` / **`Jsoup.connect(url).get()`**（同步拉页→Document，供后续选择器）；Elements `.text`/`.attr`/`.select` |
