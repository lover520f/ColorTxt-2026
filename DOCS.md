...
macOS / Linux 多架构时，`artifactName` 含 `${arch}`，`publish.channel` 为 `latest-${arch}`，避免更新描述文件互相覆盖。`build.beforePack` 会裁剪 `node_modules`；`build.onNodeModuleFile` 在 electron-builder 收集依赖时再次排除其他平台的原生包（解决 Linux x64 CI 包体膨胀）。各 build job 上传的安装包与 `*.yml` 由 **Publish** job 汇总后执行 `electron-builder publish --files …` 发布；Actions 里 Windows 的 200MB+ artifact 是 Setup + Portable 两个 exe 的传输包，Release 页面上仍是两个独立文件。macOS 未配置签名证书时以未签名包发布（`CSC_IDENTITY_AUTO_DISCOVERY=false`）。
