...
- **`readerAnnotationDecor.ts`**：标注视口 **inline 装饰**（马克笔 / 波浪线 / 直线）与按当前标注色表生成的**动态 CSS 规则**；构建 hit 索引供点击命中。
...
- **`textConvertApply.ts`**：阅读展示层与编辑态全文转换编排：**`applyTextDisplayConverts`**（简繁经 preload **`convertTextOpenCc`**，字母/数字用 **`@shared/textWidthConvert`**）；编辑态分项 **`applyTextConvertZh`** / **`applyTextConvertLetters`** / **`applyTextConvertDigits`**。
...
- **`readerAnnotations.ts`**：标注物理范围 ↔ Monaco 范围、列表行构建（`buildAnnotationListRows`）、**按章节分组**（`groupAnnotationListRowsByChapter`）、`normalizeReaderAnnotations`、正文变更后的 **`stale`** 标记逻辑。
- **`readerAnnotationExport.ts`**：标注 **JSON**（`schemaVersion: 1`）与 **Markdown** 导出/导入；默认文件名 `notes-{日期}-{书名}.md|json`；Markdown 按章节 `##` 分组、笔记 **`💡`** + 原文 blockquote、纯划线 **`✨`**、文末 `*导出于 …*`。
...
- **选区工具条**（**只读**）：选中文本或点击已有标注时出现 **`ReaderSelectionToolbar.vue`**，由 **`useReaderAnnotations.ts`** 驱动；**阅读器编辑模式**下不启用。
...
| ---- | ---- |
| `id` | 稳定 UUID |
| `startPhysicalLine` / `startColumn` / `endPhysicalLine` / `endColumn` | 源文件**物理行**范围（含空行；与书签行号语义一致） |
| `text` | 创建/更新时截取的原文快照 |
| `lineation?` | 可选划线：`type` 为 `marker`（马克笔）/ `wavy`（波浪线）/ `straight`（直线）；`colorIndex` 对应当前主题标注色表下标 |
| `note?` | 可选笔记：`content`、`createdAt`、`updatedAt` |
| `stale?` | 正文变更后与快照不一致时标记；侧栏显示「已失效」，可通过 **「清除失效笔记」** 批量移除 |
...
- **视口装饰**：**`readerAnnotationDecor.ts`** 仅为视口附近标注创建 inline decoration，并按标注色表注入 CSS；滚动时重建索引与装饰。
...
- 组内、组间均按**物理行号**排序。
- **右键**：移除单条。
- **更多菜单**：导出 Markdown / JSON、导入 JSON、清除失效笔记、清空全部（经 `App.vue` + `readerAnnotationExport.ts`）。
...
| 模块 | 职责 |
| ---- | ---- |
| **`composables/useReaderAnnotations.ts`** | 工具条 / 笔记面板 / hit 索引 / upsert-remove 上抛 |
| **`utils/readerAnnotations.ts`** | 范围运算、normalize、列表行、章节分组 |
| **`reader/readerAnnotationDecor.ts`** | Monaco inline 装饰 + 动态 CSS |
| **`utils/readerAnnotationExport.ts`** | JSON / Markdown 导出导入 |
